...
**Reviewer B (second pass):** Confirmed file exists on disk (16,726 bytes).
Agreed with all of A's findings. Added 4 observations A missed — but critically,
B independently verified the L1 "MISSING" flag was a false negative before agreeing
with A's diagnosis. See the **L1 False-Negative Trap** section below.
...
### The L1 False-Negative Trap (observed 2026-08-22)

**Session**: POST /api/relay/run spec review — Reviewer A issued CHANGES_REQUESTED
based on L1 Universal Check reporting `runtime/container_app.py MISSING`.

**What happened**: Reviewer A built an entire rejection on the L1 flag without
verifying file existence. The file was present (16,726 bytes, 61 lines changed in
git diff, both spec-required edits present). L1 "MISSING" was a false negative.

**What Reviewer B did**: Independently verified against the actual filesystem and
git state before any conclusion:
1. `ls -la runtime/container_app.py` — file exists, 16,726 bytes
2. `git diff HEAD --stat` — 61 lines changed in container_app.py
3. `git diff HEAD -- runtime/container_app.py` — both spec edits present
4. Ran all 7 spec verification steps (import, test_client, auth, DB, thread) — all PASS
5. Found A missed 6 additional modified files violating single-file delta discipline

**Lesson encoded**: L1 checks are heuristics, not ground truth. A "MISSING" flag means
"the check couldn't find it", not "the file doesn't exist". Reviewer B's job is to be
the empirical floor — verify against the actual filesystem and git state before any
conclusion. Reviewer A is the less competent model; B must not inherit A's unverified
premises. This is the same principle as the "default to success" antipattern (see
`default_to_success_antipattern.md`): claims must be verified by evidence, not assumed
by absence of verification.
