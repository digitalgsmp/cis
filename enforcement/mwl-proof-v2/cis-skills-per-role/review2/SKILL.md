...
Eric's explicit correction: "the container is meant to ensure it cooperates. why does it have an option."
...
> **See also**: `references/verify-snapshot-gap.md` — execution-start state is in-memory only, lost on resume. Detailed spec review findings (13 findings), git-apply fallback requirement, run-scoped untracked file handling, and reviewer methodology for vetting drafter specs.
...
- ✅ Step 5: pipeline_relay.py — built, tested, and **PROVEN end-to-end** (2026-07-08, commit b888d5f).
  ...
  **Full end-to-end test completed**: Brain→IntentReview→Draft (objections→revision)→ProposalReview (consensus)→ERIC_GATE (approved via API)→Menter→Verify (PASS)→CONSENSUS_REACHED.
  Run `run-892e86ca512a0056-1783535751` — 9 rounds, 11 trajectories, Verify ran git diff as evidence. Repairable gates work (reviewers objected r1 → revision → consensus r2).
  Added `_resolve_api_key()` (see [Gateway API Key Discovery](references/gateway_api_key_discovery.md)), `WAITING_FOR_HUMAN` state, `start_sync()` for Flask API, round_number auto-increment fix, per-role `AGENT_TIMEOUTS`.
...
- **[Container App Routing Verification](references/container-app-routing-verification.md)** — Werkzeug static-vs-dynamic route priority, the /api/relay/<run_id> catch-all pitfall, health-check endpoint conventions, and the 5-step verification procedure for adding static routes
