...
- **[Skill Split Per-Profile](references/skill-split-per-profile.md)** — The six gateway profiles each have their own HERMES_HOME (/home/worker/.hermes-<role>) with their own copy of this skill and their own references/pitfalls.md. Verification procedure (gateway PID → /proc environ → HERMES_HOME mapping, inode independence, writability checks) and the propagation pitfalls: repo baked copy at enforcement/mwl-proof-v2/ is still pre-split and would regress the split on image rebuild; skill updates must be applied to all six homes manually.
