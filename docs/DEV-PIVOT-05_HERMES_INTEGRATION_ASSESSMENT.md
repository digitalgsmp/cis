...
## 10. Session Handoff → See AGENTS.md §13

The session handoff narrative is now stored in the SQLite spine (`session_handoffs` table)
and auto-generated into AGENTS.md §13 on every commit. This eliminates manual DEV-PIVOT
section drift — the handoff stays current with every regeneration.

**For the latest handoff:** read AGENTS.md §13 (Session Handoff from spine).

**For Claude instructions:** see `docs/PHASE1_ROOT_DIRECTIVE_FOR_CLAUDE.md` — Tasks 1, 3, 4 active; Tasks 2 and 5 deferred per DEV-PIVOT-06 §6.
